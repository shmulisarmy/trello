...
import { ws, server_stores } from "./server_comute";

function EditSvg() {
  return (
    <svg class="hidden group-hover:block absolute top-0 right-0" width="800px" height="800px" viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function EditField({ value, onChange }: { value: string; onChange: (value: string) => void; }) {
  const [editing, set_editing] = createSignal(false);
  ...
  return (
    <Show when={editing()} fallback={<div class="group relative" onClick={() => set_editing(true)}>
      <EditSvg />
      <p>
        {value}
      </p>
      ...
      <div>
        <input ref={inputRef} type="text" value={value} onchange={(e) => onChange(e.currentTarget.value)}
          onBlur={() => set_editing(false)} />
      </div>
    </Show>
  );
}
function ServerStore_C({ store, store_name }: { store: string; store_name: string; }) {
  const ServerField = ([key, value]: [string, any]) => (
  ...
  return (
    <div>
      <For each={Object.entries(server_stores[store])}>
        {ServerField}
      </For>
      <form
        onSubmit={(e) => {
      ...
      >
        ...
        <input class="border border-blue-500 rounded" type="text" name="key" />
        <br />
        ...
        <br />
        <button
          class="border text-white p-2 cursor-pointer bg-blue-500 rounded"
          type="submit"
        >
          create new entry
        </button>
...
export function Stores_C({ server_stores }: { server_stores: {}; }) {
  return (
    <>
      <For each={Object.entries(server_stores)}>
        {([key, value]: [string, any]) => <div class="m-2 p-2 border border-slate-600 rounded shadow-sm w-fit">
          <h1>{key} store</h1>
          <ServerStore_C store={key} store_name={key} />
        </div>}
      </For>
    </>
  );
