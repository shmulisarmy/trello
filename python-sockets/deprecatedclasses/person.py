...
class nameSpace_DB:
    ...
    @staticmethod
    def get_cursor():
        """ while async this program is still single threaded so this will work"""
        try:
            return nameSpace_DB.connection.cursor()
        except psycopg2.OperationalError:
            nameSpace_DB.connection = psycopg2.connect(environ.get("DATABASE_URL"))
            return nameSpace_DB.connection.cursor()

    @staticmethod
    @db_router.get("/get_family/{parent_id}")
    def get_family(parent_id: int) -> dict:
        cursor = nameSpace_DB.get_cursor()
    ...
    class namespace_Person:
        ...
        @staticmethod
        @db_router.post("/delete_person/{person_id}", response_class=JSONResponse)
        ...
        @staticmethod
        @db_router.get("/hello", response_class=JSONResponse)
        async def hello():
